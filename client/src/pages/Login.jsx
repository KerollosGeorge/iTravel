    ...
    if (cred.userName === "" || cred.password === "") {
      ...
      try {
        const response = await axios.post(
          "https://itravel-apis.vercel.app/api/auth/login",
          cred
        );
        dispatch({
          type: "LOGIN_SUCCESS",
  ...
  return (
    <div className='flex justify-center items-center w-[100%] h-[100vh] bg-gray-300 max-sm:flex max-sm:flex-col gap-1 bg-[url("https://digital.ihg.com/is/image/ihg/ihgor-member-rate-web-offers-1440x720")] bg-cover'>
      ...
      <form className="relative flex flex-col min-w-[400px] h-[500px] max-sm:h-[400px] bg-white shadow-gray-800 space-y-[50px] max-sm:space-y-[40px] p-5 rounded-md">
        <h2 className="text-center text-3xl text-black">Login now</h2>
        <div className="flex flex-col gap-5">
          <div className="flex flex-col w-full text-black">
            ...
            <input
              ...
              className="w-[250px] text-black h-9 border-b-2 border-blue-900 outline-none indent-[7px] leading-tight focus:outline-none focus:shadow-outline"
              onChange={handleChange}
              required
            ></input>
          </div>
          <div className="relative flex flex-col w-[250px] text-black">
        ...
        <div className="absolute bottom-10 left-0 w-full flex justify-around">
          ...
          <p>
            create acount{" "}
            <Link className="text-sm text-blue-700" to="/register">
              sign up
            </Link>
          </p>
        </div>
